...
export const createPost = async (req, res) => {
  ...
  try {
   ...
   req.app.get('socketio').emit('message_from_posts', {text:`user fff has created post `, uid:uid})
...
export const createComment = async (req, res) => {
 ...
 usersarr.map((t)=>{ console.log(t._id+":::"+u_id)})
  req.app.get('socketio').to(socketids.get(   usersarr[    usersarr.findIndex((t)=>{return Number(t._id)===Number(u_id)})]._id)   )
    .emit('message_from_comments', {text:`user ${ usersarr[  usersarr.findIndex((t)=>{return Number(t._id)===Number(u_id)})].email}
    commented your post ${posts[posts.findIndex((t)=>{return Number(t._id)===Number(post_id)} )].description} `, flag:1, uid:u_id})
